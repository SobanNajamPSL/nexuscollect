...
Every screen shows a small badge in the top-right corner:

> **Demo clock: 2026-07-30 (Asia/Karachi)**

This is not decorative. The platform never reads your computer's or server's real clock for any business decision — every date-sensitive calculation (is this bill overdue? is the early-payment discount still live? has a cheque cleared?) is driven by this single, injected clock value. In the demonstration environment it is fixed to 30 July 2026 so that the same walkthrough produces the same numbers today, tomorrow, or a year from now. In a live deployment, this would simply track the real current time — the mechanism is identical either way.

## What's next

Continue to [Screen 1 — Citizen Payment](01-citizen-payment.md) to begin the walkthrough, or jump directly to any screen using the table of contents in the [README](README.md).
